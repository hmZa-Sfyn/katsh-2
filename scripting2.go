...
func (sh *Shell) evalThrow(raw string) int {
	// Strip "throw " or "raise " prefix
	lraw := strings.ToLower(raw)
	msg := ""
	if strings.HasPrefix(lraw, "throw ") {
		msg = strings.TrimSpace(raw[6:])
	} else if strings.HasPrefix(lraw, "raise ") {
		msg = strings.TrimSpace(raw[6:])
	}
	...
	if sh.errHandlerDepth == 0 {
		...
		PrintError(&ShellError{
			...
			Col:     -1,
			Detail:  detail,
			Hint:    "wrap throwing code in:  try { ... } catch e { println $e }",
...
func (sh *Shell) evalScript2(raw string) (bool, int) {
	...
	// Standalone data type display:  map_show $m  /  dt_show $m
	// (already handled by builtin dispatch — but let's catch
	//  data type literals used as standalone expressions: map { ... })
